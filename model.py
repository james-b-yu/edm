...
class EGCL(nn.Module):
    def __init__(self, features_d: int, node_attr_d: int, edge_attr_d: int, output_d: int, hidden_d: int, act_fn = nn.SiLU()):
        super().__init__()
        
        # the \(\phi_{e}\) network for edge operation
        self.edge_mlp = nn.Sequential(
            #            hi               hj            dij2         aij
            nn.Linear(features_d   +   features_d   +   1   +   edge_attr_d, hidden_d),
            act_fn,
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(features_d + hidden_d + node_attr_d, hidden_d),
            act_fn,
    ...
    def forward(self, coords: torch.Tensor, features: torch.Tensor, edges: torch.Tensor, reduce: torch.Tensor, node_attr: torch.Tensor|None=None, edge_attr: torch.Tensor|None=None):
        ...
        coord_mlp_in = edge_mlp_in = torch.cat([x_e, h_e_flattened, d_e ** 2], dim=-1) if edge_attr is None else torch.cat([x_e, h_e_flattened, d_e ** 2, edge_attr], dim=-1)
        ...
        node_mlp_in = torch.cat([features, reduce @ (phi_e * self.inf_mlp(phi_e))], dim=-1) if node_attr is None else torch.cat([features, reduce @ (phi_e * self.inf_mlp(phi_e)), node_attr], dim=-1)
        features_out: torch.Tensor = self.node_mlp(node_mlp_in)
        
        # calculate coord vector output
        w = phi_x / (d_e + 1)
        coords_out: torch.Tensor = coords + reduce @ (w * (x_e[:, 0, :] - x_e[:, 1, :]))
        
        return features_out, coords_out