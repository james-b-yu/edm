...
class EDMDatasetItem:
    """Attributes:
        ...
        features (torch.Tensor): [n, features_d] giving the features (h) of each atom in the molecule
    """
    n_nodes: int
    coords: torch.Tensor
...
class EDMDataloaderItem:
    """
    A class representing a batch of molecules. Note that we use "flattened" notation, in that coords and features are one long tensor of size [N, 3] or [N, features_d] respectively, where N is the number of atoms across the entire batch. This allows each molecule to have a different number of atoms and we keep track of this dynamically.
...
class DummyDataset(EDMDataset):
    """Dummy dataset
    """
    def __init__(self, features_d=7, max_nodes=25, len=10000):
        super().__init__(features_d, max_nodes)
        self.len = len
    ...
    def __getitem__(self, index) -> EDMDatasetItem:
        ...
        features = torch.randn((n_nodes, self.features_d), dtype=torch.float32)
